...
// emitBootPagerHTMLBody emits the pagination navigation bar IR (v0.9.0 P1.7,
// mirrors Go boot.PagerHTML / Page.HTMLPager):
//
//	ptr @__kylix_boot_pager_html(ptr %base, i64 %page, i64 %size,
//	                             i64 %total, i64 %window)
//
// Structure: `<nav class="pager">‹ Prev [1] … [n] … [pages] Next ›</nav>` —
// pager-prev/pager-current/pager-link/pager-ellipsis/pager-disabled classes,
// URLs are `base + ("?"|"&") + "page=N"` (no query merge — pass a queryless
// base, see TECHNICAL_DEBT), hrefs are HTML-attribute-escaped. The result is
// built on the per-request arena.
//
// Helpers: pager_put (append str), pager_puti (append int), pager_url (build
// one page URL), htmlattr (escape & " < >), pager_link (one numbered link).
func (g *Generator) emitBootPagerHTMLBody() {
	c := func(format string, args ...interface{}) { g.line(fmt.Sprintf(format, args...)) }
	g.needArena = true
	// lit adds a string constant and emits its GEP with the exact size —
	// ptrTo's size must match the constant's [N x i8] type (len+1 for NUL),
	// and hand-counting breaks on quotes/UTF-8.
	lit := func(s string) string { return g.ptrTo(g.addString(s), len(s)+1) }

	// put: append a NUL-terminated string at the cursor, return the new one.
	g.line("define ptr @__kylix_boot_pager_put(ptr %cur, ptr %s) {")
	g.line("entry:")
	g.line("  call ptr @strcat(ptr %cur, ptr %s)")
	pn := g.tmp()
	c("  %s = call i64 @strlen(ptr %%s)", pn)
	pnext := g.tmp()
	c("  %s = getelementptr inbounds i8, ptr %%cur, i64 %s", pnext, pn)
	c("  ret ptr %s", pnext)
	g.line("}")
	g.line("")

	// puti: append an integer via snprintf, return the advanced cursor.
	g.line("define ptr @__kylix_boot_pager_puti(ptr %cur, i64 %n) {")
	g.line("entry:")
	fmtI := lit("%lld")
	pr := g.tmp()
	c("  %s = call i32 (ptr, i64, ptr, ...) @snprintf(ptr %%cur, i64 24, ptr %s, i64 %%n)", pr, fmtI)
	pni := g.tmp()
	c("  %s = sext i32 %s to i64", pni, pr)
	pnexti := g.tmp()
	c("  %s = getelementptr inbounds i8, ptr %%cur, i64 %s", pnexti, pni)
	c("  ret ptr %s", pnexti)
	g.line("}")
	g.line("")

	// url: base + ("?"|"&") + "page=N" (malloc'd). Two format constants
	// selected by whether the base already contains a '?'.
	g.line("define ptr @__kylix_boot_pager_url(ptr %base, i64 %n) {")
	g.line("entry:")
	q := g.tmp()
	c("  %s = call ptr @strchr(ptr %%base, i32 63)", q)
	isQ := g.tmp()
	c("  %s = icmp ne ptr %s, null", isQ, q)
	fmtAmp := lit("%s&page=%lld")
	fmtQ := lit("%s?page=%lld")
	fSel := g.tmp()
	c("  %s = select i1 %s, ptr %s, ptr %s", fSel, isQ, fmtAmp, fmtQ)
	bl := g.tmp()
	c("  %s = call i64 @strlen(ptr %%base)", bl)
	ucap := g.tmp()
	c("  %s = add i64 %s, 32", ucap, bl)
	ubuf := g.tmp()
	c("  %s = %s", ubuf, g.mallocCall(ucap))
	c("  call i32 (ptr, i64, ptr, ...) @snprintf(ptr %s, i64 %s, ptr %s, ptr %%base, i64 %%n)", ubuf, ucap, fSel)
	c("  ret ptr %s", ubuf)
	g.line("}")
	g.line("")

	// htmlattr: escape & " < > for double-quoted attribute values (one pass,
	// worst case 6 bytes per input byte).
	g.line("define ptr @__kylix_boot_htmlattr(ptr %s) {")
	g.line("entry:")
	hbl := g.tmp()
	c("  %s = call i64 @strlen(ptr %%s)", hbl)
	hcap := g.tmp()
	c("  %s = mul i64 %s, 6", hcap, hbl)
	hcap1 := g.tmp()
	c("  %s = add i64 %s, 1", hcap1, hcap)
	out0 := g.tmp()
	c("  %s = %s", out0, g.mallocCall(hcap1))
	outSlot := g.tmp()
	c("  %s = alloca ptr, align 8", outSlot)
	c("  store ptr %s, ptr %s", out0, outSlot)
	srcSlot := g.tmp()
	c("  %s = alloca ptr, align 8", srcSlot)
	g.line("  store ptr %s, ptr " + srcSlot)
	// one arm per escapable byte: memcpy the entity, advance both cursors.
	arm := func(ch, src string, testByte int64, entity string, entityLen int, nextLbl string) {
		eq := g.tmp()
		c("  %s = icmp eq i8 %s, %d", eq, ch, testByte)
		writeLbl := g.label()
		c("  br i1 %s, label %%%s, label %%%s", eq, writeLbl, nextLbl)
		c("%s:", writeLbl)
		out := g.tmp()
		c("  %s = load ptr, ptr %s", out, outSlot)
		g.line(fmt.Sprintf("  call ptr @memcpy(ptr %s, ptr %s, i64 %d)", out, g.ptrTo(g.addString(entity), entityLen), entityLen))
		outAdv := g.tmp()
		c("  %s = getelementptr inbounds i8, ptr %s, i64 %d", outAdv, out, entityLen-1)
		c("  store ptr %s, ptr %s", outAdv, outSlot)
		srcAdv := g.tmp()
		c("  %s = getelementptr inbounds i8, ptr %s, i64 1", srcAdv, src)
		c("  store ptr %s, ptr %s", srcAdv, srcSlot)
		g.line("  br label %scan")
	}
	g.line("  br label %scan")
	g.line("scan:")
	src := g.tmp()
	c("  %s = load ptr, ptr %s", src, srcSlot)
	ch := g.tmp()
	c("  %s = load i8, ptr %s", ch, src)
	hdone := g.tmp()
	c("  %s = icmp eq i8 %s, 0", hdone, ch)
	finLbl := g.label()
	c("  br i1 %s, label %%%s, label %%c_amp", hdone, finLbl)
	g.line("c_amp:")
	arm(ch, src, 38, "&amp;", 6, "c_quot")
	g.line("c_quot:")
	arm(ch, src, 34, "&quot;", 7, "c_lt")
	g.line("c_lt:")
	arm(ch, src, 60, "&lt;", 5, "c_gt")
	g.line("c_gt:")
	arm(ch, src, 62, "&gt;", 5, "h_plain")
	g.line("h_plain:")
	hout := g.tmp()
	c("  %s = load ptr, ptr %s", hout, outSlot)
	c("  store i8 %s, ptr %s", ch, hout)
	houtAdv := g.tmp()
	c("  %s = getelementptr inbounds i8, ptr %s, i64 1", houtAdv, hout)
	c("  store ptr %s, ptr %s", houtAdv, outSlot)
	hsrcAdv := g.tmp()
	c("  %s = getelementptr inbounds i8, ptr %s, i64 1", hsrcAdv, src)
	c("  store ptr %s, ptr %s", hsrcAdv, srcSlot)
	g.line("  br label %scan")
	g.line(fmt.Sprintf("%s:", finLbl))
	houtF := g.tmp()
	c("  %s = load ptr, ptr %s", houtF, outSlot)
	g.line("  store i8 0, ptr " + houtF)
	c("  ret ptr %s", out0)
	g.line("}")
	g.line("")

	// link: <a class="pager-link" href="ESCAPED">N</a>
	g.line("define ptr @__kylix_boot_pager_link(ptr %cur, ptr %href, i64 %n) {")
	g.line("entry:")
	esc := g.tmp()
	c("  %s = call ptr @__kylix_boot_htmlattr(ptr %%href)", esc)
	linkA := lit(`<a class="pager-link" href="`)
	linkB := lit(`">`)
	linkC := lit(`</a>`)
	l1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %%cur, ptr %s)", l1, linkA)
	l2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", l2, l1, esc)
	l3 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", l3, l2, linkB)
	l4 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_puti(ptr %s, i64 %%n)", l4, l3)
	l5 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", l5, l4, linkC)
	c("  ret ptr %s", l5)
	g.line("}")
	g.line("")

	// main: assemble the nav bar.
	g.line("define ptr @__kylix_boot_pager_html(ptr %base, i64 %page, i64 %size, i64 %total, i64 %window) {")
	g.line("entry:")
	// pages = size>0 ? max(1, (total+size-1)/size) : 1
	szPos := g.tmp()
	c("  %s = icmp sgt i64 %%size, 0", szPos)
	num := g.tmp()
	c("  %s = add i64 %%total, %%size", num)
	num1 := g.tmp()
	c("  %s = sub i64 %s, 1", num1, num)
	dv := g.tmp()
	c("  %s = sdiv i64 %s, %%size", dv, num1)
	dvLt1 := g.tmp()
	c("  %s = icmp slt i64 %s, 1", dvLt1, dv)
	pages0 := g.tmp()
	c("  %s = select i1 %s, i64 1, i64 %s", pages0, dvLt1, dv)
	pages := g.tmp()
	c("  %s = select i1 %s, i64 %s, i64 1", pages, szPos, pages0)
	hasPrev := g.tmp()
	c("  %s = icmp sgt i64 %%page, 1", hasPrev)
	hasNext := g.tmp()
	c("  %s = icmp slt i64 %%page, %s", hasNext, pages)
	pm1 := g.tmp()
	c("  %s = sub i64 %%page, 1", pm1)
	pgLt2 := g.tmp()
	c("  %s = icmp slt i64 %%page, 2", pgLt2)
	prev := g.tmp()
	c("  %s = select i1 %s, i64 1, i64 %s", prev, pgLt2, pm1)
	pp1 := g.tmp()
	c("  %s = add i64 %%page, 1", pp1)
	nextGt := g.tmp()
	c("  %s = icmp sgt i64 %s, %s", nextGt, pp1, pages)
	next := g.tmp()
	c("  %s = select i1 %s, i64 %s, i64 %s", next, nextGt, pages, pp1)
	lo0 := g.tmp()
	c("  %s = sub i64 %%page, %%window", lo0)
	loLt := g.tmp()
	c("  %s = icmp slt i64 %s, 1", loLt, lo0)
	lo := g.tmp()
	c("  %s = select i1 %s, i64 1, i64 %s", lo, loLt, lo0)
	hi0 := g.tmp()
	c("  %s = add i64 %%page, %%window", hi0)
	hiGt := g.tmp()
	c("  %s = icmp sgt i64 %s, %s", hiGt, hi0, pages)
	hi := g.tmp()
	c("  %s = select i1 %s, i64 %s, i64 %s", hi, hiGt, pages, hi0)
	// cap = 2048 + (2*window+8)*(baseLen+96)
	baseLen := g.tmp()
	c("  %s = call i64 @strlen(ptr %%base)", baseLen)
	w2 := g.tmp()
	c("  %s = add i64 %%window, %%window", w2)
	w3 := g.tmp()
	c("  %s = add i64 %s, 8", w3, w2)
	blc := g.tmp()
	c("  %s = add i64 %s, 96", blc, baseLen)
	linksCap := g.tmp()
	c("  %s = mul i64 %s, %s", linksCap, w3, blc)
	cap := g.tmp()
	c("  %s = add i64 %s, 2048", cap, linksCap)
	buf := g.tmp()
	c("  %s = call ptr @__kylix_arena_alloc(i64 %s)", buf, cap)
	// Arena memory is recycled per request and NOT zeroed — strcat's cursor
	// append semantics need buf[0]==0 or the first put appends past stale
	// bytes from the previous request (corrupted nav on request 2+).
	g.line("  store i8 0, ptr " + buf)
	curSlot := g.tmp()
	c("  %s = alloca ptr, align 8", curSlot)
	c("  store ptr %s, ptr %s", buf, curSlot)
	iSlot := g.tmp()
	c("  %s = alloca i64, align 8", iSlot)

	navLit := lit(`<nav class="pager">`)
	startCur := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", startCur, buf, navLit)
	c("  store ptr %s, ptr %s", startCur, curSlot)
	c("  br i1 %s, label %%p_link, label %%p_span", hasPrev)
	g.line("p_link:")
	u1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_url(ptr %%base, i64 %s)", u1, prev)
	e1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_htmlattr(ptr %s)", e1, u1)
	prevA := lit(`<a class="pager-prev" href="`)
	prevB := lit(`">‹ Prev</a>`)
	q1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", q1, startCur, prevA)
	q2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", q2, q1, e1)
	q3 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", q3, q2, prevB)
	c("  store ptr %s, ptr %s", q3, curSlot)
	g.line("  br label %p_done")
	g.line("p_span:")
	prevD := lit(`<span class="pager-prev pager-disabled">‹ Prev</span>`)
	q4 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", q4, startCur, prevD)
	c("  store ptr %s, ptr %s", q4, curSlot)
	g.line("  br label %p_done")
	g.line("p_done:")
	c2 := g.tmp()
	c("  %s = load ptr, ptr %s", c2, curSlot)
	loGt1 := g.tmp()
	c("  %s = icmp sgt i64 %s, 1", loGt1, lo)
	c("  br i1 %s, label %%first, label %%mid", loGt1)
	g.line("first:")
	u2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_url(ptr %%base, i64 1)", u2)
	f1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_link(ptr %s, ptr %s, i64 1)", f1, c2, u2)
	c("  store ptr %s, ptr %s", f1, curSlot)
	loGt2 := g.tmp()
	c("  %s = icmp sgt i64 %s, 2", loGt2, lo)
	c("  br i1 %s, label %%ell1, label %%mid", loGt2)
	g.line("ell1:")
	ellLit := lit(`<span class="pager-ellipsis">…</span>`)
	f2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", f2, f1, ellLit)
	c("  store ptr %s, ptr %s", f2, curSlot)
	g.line("  br label %mid")
	g.line("mid:")
	c3 := g.tmp()
	c("  %s = load ptr, ptr %s", c3, curSlot)
	c("  store i64 %s, ptr %s", lo, iSlot)
	g.line("  br label %loop")
	g.line("loop:")
	nReg := g.tmp()
	c("  %s = load i64, ptr %s", nReg, iSlot)
	inLoop := g.tmp()
	c("  %s = icmp sle i64 %s, %s", inLoop, nReg, hi)
	c("  br i1 %s, label %%body, label %%after", inLoop)
	g.line("body:")
	isCur := g.tmp()
	c("  %s = icmp eq i64 %s, %%page", isCur, nReg)
	c("  br i1 %s, label %%curB, label %%lnk", isCur)
	g.line("curB:")
	c4 := g.tmp()
	c("  %s = load ptr, ptr %s", c4, curSlot)
	curA := lit(`<span class="pager-current">`)
	curB := lit(`</span>`)
	b1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", b1, c4, curA)
	b2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_puti(ptr %s, i64 %s)", b2, b1, nReg)
	b3 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", b3, b2, curB)
	c("  store ptr %s, ptr %s", b3, curSlot)
	g.line("  br label %inc")
	g.line("lnk:")
	c5 := g.tmp()
	c("  %s = load ptr, ptr %s", c5, curSlot)
	u3 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_url(ptr %%base, i64 %s)", u3, nReg)
	b4 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_link(ptr %s, ptr %s, i64 %s)", b4, c5, u3, nReg)
	c("  store ptr %s, ptr %s", b4, curSlot)
	g.line("  br label %inc")
	g.line("inc:")
	n1 := g.tmp()
	c("  %s = add i64 %s, 1", n1, nReg)
	c("  store i64 %s, ptr %s", n1, iSlot)
	g.line("  br label %loop")
	g.line("after:")
	c6 := g.tmp()
	c("  %s = load ptr, ptr %s", c6, curSlot)
	hiLtPages := g.tmp()
	c("  %s = icmp slt i64 %s, %s", hiLtPages, hi, pages)
	c("  br i1 %s, label %%lastChk, label %%nchk", hiLtPages)
	g.line("lastChk:")
	pmPages := g.tmp()
	c("  %s = sub i64 %s, 1", pmPages, pages)
	hiLtPm := g.tmp()
	c("  %s = icmp slt i64 %s, %s", hiLtPm, hi, pmPages)
	c("  br i1 %s, label %%ell2, label %%lastLink", hiLtPm)
	g.line("ell2:")
	lE1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", lE1, c6, ellLit)
	c("  store ptr %s, ptr %s", lE1, curSlot)
	g.line("  br label %lastLink")
	g.line("lastLink:")
	c7 := g.tmp()
	c("  %s = load ptr, ptr %s", c7, curSlot)
	u4 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_url(ptr %%base, i64 %s)", u4, pages)
	lE2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_link(ptr %s, ptr %s, i64 %s)", lE2, c7, u4, pages)
	c("  store ptr %s, ptr %s", lE2, curSlot)
	g.line("  br label %nchk")
	g.line("nchk:")
	c8 := g.tmp()
	c("  %s = load ptr, ptr %s", c8, curSlot)
	c("  br i1 %s, label %%n_link, label %%n_span", hasNext)
	g.line("n_link:")
	u5 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_url(ptr %%base, i64 %s)", u5, next)
	e2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_htmlattr(ptr %s)", e2, u5)
	nextA := lit(`<a class="pager-next" href="`)
	nextB := lit(`">Next ›</a>`)
	x1 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", x1, c8, nextA)
	x2 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", x2, x1, e2)
	x3 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", x3, x2, nextB)
	c("  store ptr %s, ptr %s", x3, curSlot)
	g.line("  br label %fin")
	g.line("n_span:")
	nextD := lit(`<span class="pager-next pager-disabled">Next ›</span>`)
	x4 := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", x4, c8, nextD)
	c("  store ptr %s, ptr %s", x4, curSlot)
	g.line("  br label %fin")
	g.line("fin:")
	c9 := g.tmp()
	c("  %s = load ptr, ptr %s", c9, curSlot)
	endLit := lit(`</nav>`)
	z := g.tmp()
	c("  %s = call ptr @__kylix_boot_pager_put(ptr %s, ptr %s)", z, c9, endLit)
	c("  ret ptr %s", buf)
	g.line("}")
	g.line("")
}
