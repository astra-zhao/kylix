...
func (g *Generator) emitCryptoHmacSha256Body() {
	...
	innerBuf := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", innerBuf, g.mallocCall(innerBufSize)))
...
func (g *Generator) emitCryptoHexbytesBody() {
	...
	out := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", out, g.mallocCall(outSize)))
...
func (g *Generator) emitCryptoAesEncryptBody() {
	...
	outBuf := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", outBuf, g.mallocCall(outSize)))
