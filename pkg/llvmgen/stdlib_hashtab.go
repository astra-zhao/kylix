...
func (g *Generator) emitHashtabKeys() {
	...
	items := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", items, g.mallocCall(nBytes)))
	bucketsSlot := g.tmp()
	...
	biSlot := g.tmp()
	g.line(fmt.Sprintf("  %s = alloca i64, align 8", biSlot))
	g.line(fmt.Sprintf("  store i64 0, ptr %s", biSlot))
	nodeSlot := g.tmp()
	...
	bucketDone := g.label()
	retLbl := g.label()
	g.line(fmt.Sprintf("  br label %%%s", outerCond))
	g.line(fmt.Sprintf("%s:", outerCond))
	bi := g.tmp()
	g.line(fmt.Sprintf("  %s = load i64, ptr %s", bi, biSlot))
	bDone := g.tmp()
	g.line(fmt.Sprintf("  %s = icmp sge i64 %s, %d", bDone, bi, htabBucketCount))
	g.line(fmt.Sprintf("  br i1 %s, label %%%s, label %%%s", bDone, retLbl, innerCond))
	g.line(fmt.Sprintf("%s:", innerCond))
	bucketPtr := g.tmp()
	g.line(fmt.Sprintf("  %s = getelementptr inbounds ptr, ptr %s, i64 %s", bucketPtr, buckets, bi))
	first := g.tmp()
	g.line(fmt.Sprintf("  %s = load ptr, ptr %s", first, bucketPtr))
	g.line(fmt.Sprintf("  store ptr %s, ptr %s", first, nodeSlot))
	g.line(fmt.Sprintf("  br label %%%s", innerCond2))
	g.line(fmt.Sprintf("%s:", innerCond2))
	node := g.tmp()
	g.line(fmt.Sprintf("  %s = load ptr, ptr %s", node, nodeSlot))
	empty := g.tmp()
	g.line(fmt.Sprintf("  %s = icmp eq ptr %s, null", empty, node))
	g.line(fmt.Sprintf("  br i1 %s, label %%%s, label %%%s", empty, bucketDone, innerBody))
	g.line(fmt.Sprintf("%s:", innerBody))
	key := g.tmp()
	g.line(fmt.Sprintf("  %s = load ptr, ptr %s", key, node))
	curIdx := g.tmp()
	g.line(fmt.Sprintf("  %s = load i64, ptr %s", curIdx, idxSlot))
	itemPtr := g.tmp()
	g.line(fmt.Sprintf("  %s = getelementptr inbounds ptr, ptr %s, i64 %s", itemPtr, items, curIdx))
	g.line(fmt.Sprintf("  store ptr %s, ptr %s", key, itemPtr))
	idxNext := g.tmp()
	g.line(fmt.Sprintf("  %s = add i64 %s, 1", idxNext, curIdx))
	g.line(fmt.Sprintf("  store i64 %s, ptr %s", idxNext, idxSlot))
	nextField := g.tmp()
	...
	g.line(fmt.Sprintf("  br label %%%s", innerCond2))
	g.line(fmt.Sprintf("%s:", bucketDone))
	biNext := g.tmp()
	g.line(fmt.Sprintf("  %s = add i64 %s, 1", biNext, bi))
	g.line(fmt.Sprintf("  store i64 %s, ptr %s", biNext, biSlot))
	g.line(fmt.Sprintf("  br label %%%s", outerCond))
	g.line(fmt.Sprintf("%s:", retLbl))
	finalIdx := g.tmp()
	g.line(fmt.Sprintf("  %s = load i64, ptr %s", finalIdx, idxSlot))
	agg0 := g.tmp()
	g.line(fmt.Sprintf("  %s = insertvalue { ptr, i64 } undef, ptr %s, 0", agg0, items))
	agg1 := g.tmp()
	g.line(fmt.Sprintf("  %s = insertvalue { ptr, i64 } %s, i64 %s, 1", agg1, agg0, finalIdx))
	g.line(fmt.Sprintf("  ret { ptr, i64 } %s", agg1))
	g.line("}")
	g.line("")
}
...
func (g *Generator) emitHashtabNew() {
	...
	g.line("entry:")
	hdr := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", hdr, g.mallocCall("24")))
	g.line(fmt.Sprintf("  store i64 %d, ptr %s", htabMagic, hdr))
	buckets := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", buckets, g.mallocCall(fmt.Sprintf("%d", htabBucketCount*8))))
