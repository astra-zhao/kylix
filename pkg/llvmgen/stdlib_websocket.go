...
func (g *Generator) emitWsRecvBody() {
	...
	g.line(fmt.Sprintf("%s:", loop))
	hdr := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", hdr, g.mallocCall("2")))
	nh := g.tmp()
	...
	ext2b := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", ext2b, g.mallocCall("2")))
	...
	ext8b := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", ext8b, g.mallocCall("8")))
	...
	mask := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", mask, g.mallocCall("4")))
	maskedBool := g.tmp()
	...
	payload := g.tmp()
	g.line(fmt.Sprintf("  %s = %s", payload, g.mallocCall(plSize)))
	npl := g.tmp()
