...
- **v6.0.0 — KylixRT 生产就绪**：klx 编译器 codegen 修复（`var` 参数值传递 → 指针、`(as T).Field` 链式）+ JsonEncode 双端 parity + CI/CD 自动化 + 性能 benchmark + LLVM -O2 优化验证 + JetBrains 插件。
