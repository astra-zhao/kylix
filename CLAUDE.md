...
- **v6.4.0**：LLVM stdlib 剩余真实现——DbQueryRows（复合类型 lowering）、websocket（协议层）+ net Winsock / regex pcre2（需 Windows 真机验证，CI runner 装不上 LLVM）+ #9 JetBrains 插件（IntelliJ SDK 大工程，单独排期）。
