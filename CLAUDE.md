...
- v6.2.0 已发布：**跨平台**——IR target triple/datalayout 参数化（`CompileOpts.Target` + `tripleFor` 表 + `emitHeader`），llc 交叉产正确 .o（ELF/COFF/aarch64）；CLI `--target` 贯通 LLVM 后端（Windows 自动 .exe）；系统库链接平台化；修 Linux PIE 重定位失败（`-no-pie`）。**Linux 51 教程 LLVM 51/51**（CI 稳定绿）+ `kylix doctor` 预检（分发形态 A）+ README LLVM 前置。Windows job 尽力而为（runner LLVM 安装 GUI/UAC 不可靠，无 llc 跳过，需有 LLVM 环境验证）。**剩余**：Windows 平台 API 适配（net Winsock/regex/datetime/exc/sysutil）、Windows 真机编译验证。
