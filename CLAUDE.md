...
- **v6.7.0**：#9 JetBrains 插件（IntelliJ/GoLand：`jetbrains-plugin/` Gradle Kotlin 模块，复用 vscode-ext tmLanguage + LSP4IJ 桥 `kylix lsp`，本机可做）+ boot server 补强（POST body 读取 / req.JSON 绑定 / BootRegisterJwtAuth 真校验）+ stdlib 补全（encoding Base64URL / httpclient JSON 嵌套）。
