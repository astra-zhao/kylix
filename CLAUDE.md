...
## 当前状态：v6.2.0（2026-08-11）

- v6.2.0 已发布：**跨平台**——IR target triple/datalayout 参数化（`CompileOpts.Target` + `tripleFor` 表 + `emitHeader`），llc 交叉产正确 .o（ELF/COFF/aarch64）；CLI `--target` 贯通 LLVM 后端（Windows 自动 .exe）；系统库链接平台化；修 Linux PIE 重定位失败（`-no-pie`）。**Linux 51 教程 LLVM 51/51**（CI 稳定绿）+ `kylix doctor` 预检（分发形态 A）+ README LLVM 前置。**平台 API 适配**（按 targetOS）：sysutil（fopen 二进制模式 + `_access`）、datetime（`localtime_s`）、exc（`_setjmp/_longjmp` + jmpbuf 512）、net/regex（Windows stub，Winsock/pcre2 待真实现）。Windows job 尽力而为（runner LLVM 安装 GUI/UAC 不可靠，无 llc 跳过）。**剩余**：net Winsock / regex pcre2 真实现（需 Windows 环境）、Windows 真机编译验证。
...
- **v6.2.0 — 跨平台 + 分发 A** ✅（2026-08-11，部分）：target triple 参数化 + CLI --target 贯通 + 系统库链接平台化 + **Linux LLVM 51/51** + `kylix doctor` + README。**剩余**：Windows 平台 API 适配（net Winsock/regex/datetime/exc/sysutil）+ Windows 真机编译验证（CI runner 装不上 LLVM）。
- **v6.2.0 续 — KylixRT 生产化**：LLVM stdlib 剩余 stub 补齐（DbQueryRows 结果集、jwt 真实实现、sysutil 文件操作、websocket）+ `kylix test/bench` LLVM 路由（镜像 run）+ 分发 B（捆绑 llc/clang，+220MiB，可选）。
