...
- v6.0.0 已发布：#7 性能 benchmark——`Result` 加 Duration/CacheHits/CacheMisses + `build --time` flag + `benchmarks/compile_time.sh` + `docs/compile-performance.md`。数据（Apple Silicon/LLVM 22）：Go 冷编译 29ms / 热 23ms / LLVM -O0 11527ms / **LLVM -O2 575ms**（opt 缩小 IR 后 llc 反而快 20×）。
...
- **v6.0.0 — 性能 benchmark（#7）** ✅（2026-08-10）：Result/BuildCache 统计字段 + `build --time` + `benchmarks/compile_time.sh` + `docs/compile-performance.md`。数据：Go 冷 29ms/热 23ms、LLVM -O0 11527ms、**-O2 575ms**。剩余：#8 LLVM -O2 教程验证（现 bootstrap -O2 已可用，教程 --llvm-opt=2 待跑）、#9 JetBrains 插件、klx codegen 修复（`var` 参数指针、`(as T).Field`）。
