...
### P2：页面渲染 API（res.HTML/Render、req.Form/Cookie、static/ 约定）

- **Go 后端（pkg/boot）**：`Request.Form(name)`（`url.ParseQuery(string(Body()))` 命中返回，否则 Query 兜底；%XX 与 `+` 自动解码）；`Request.Cookie(name)`；`Response.Html(body)`（text/html; charset=utf-8）；`Response.Cookies []string` + `WithCookie(name, value)`（发 `name=value; Path=/`，ServeHTTP 逐条 Add("Set-Cookie")）；**静态文件**：`Router.StaticDir` + `SetStaticDir`（`boot.SetStaticDir` 桥接），GET `/static/<rel>` 经 `path.Clean("/"+rel)` 防 `..` 遍历后 os.ReadFile，扩展名 MIME 表（html/css/js/png/... 16 项，默认 application/octet-stream），miss 落回 404、路由优先于静态。
- **LLVM 后端（pkg/llvmgen）双端同步**：**TResponse handle 升级 40B** `{i64 status@0, ptr body@8, ptr ctype@16, ptr cookie@24, ptr xhdrs@32}`（BootText→text/plain、BootHTML→text/html、BootJSON→application/json）；**fluent 方法 6 个**（emitBootResponseMethodCall）：Html/Send（Send 时 ctype null→text/plain select）/StatusCode/WithCookie（snprintf `"%s=%s; Path=/"`）/WithHeader（xhdrs null→malloc 1024 + strcat `"%s: %s\r\n"`）；`req.Form`/`req.Cookie` 内联 `__kylix_boot_form_get`（entry 后 %XX 十六进制解码 `__kylix_boot_hexval` select 链 + '+'→32）与 `__kylix_boot_cookie_get`（strstr "Cookie:" + 分号对扫描 + ptrtoint 差算 keyLen）；**BootStatic**（`void @__kylix_boot_BootStatic(ptr)` 存 `@__kylix_boot_static_dir` 全局）+ BootRun 路由 miss → `i1 @__kylix_boot_serve_static`（strncmp "/static/" 8 字节 → fopen/fseek/ftell/fread/fclose → strrchr '.' → MIME select 链 11 项 → send 头+体二进制安全）→ 失败才 404；BootRun 响应构建补 Content-Type（res+16，null→text/html 默认）+ 自定义头（xhdrs）+ Set-Cookie（res+24）；per-request arena 2048→8192。
- **声明与测试**：`stdlib/klx/web.klx` 重写（TRequest Param/Query/Header/Body/Form/Cookie + TResponse Html/Send/StatusCode/WithHeader/WithCookie，LSP 补全用）；`stdlib/klx/boot.klx` 补 `procedure BootStatic(dir: String)`；llvmgen 新增 stdlib_boot_pages_test.go（响应 40B handle/静态 IR/表单+cookie IR 3 测试）+ pkg/boot 新增 8 测试（Form 解码/Query 兜底/Cookie/Html/WithCookie wire/静态 MIME+嵌套+404 兜底+路由优先/遍历防护）。
- **验证**：16 包全绿；Go/LLVM 52 教程 52/52；bootstrap sweep **52 PASS + 1 SKIP**（example59 多文件首入全绿——此前 sweep 报 FAIL 系 BOOT 二进制过期，重建后消失）；端到端 page.klx（BootHTML + fluent 链 + Form/Cookie + BootStatic）host LLVM `✓ Built`；**curl 六场景运行时验证全过**（fluent 链 201+自定义头+Set-Cookie 落 header、`Li%20Si`→`Li Si`、`%41%42C`→`ABC`、静态 css 200+text/css、未命中 404、Cookie 头解析）。
- **运行时破案修复 5 处**（curl 验证暴露）：① form_get dpct 分支 `%XY` 高低半字节写反（hexval(X) 应 `shl 4`，曾解出 `Li\x02Si` 不可见控制符）；② serve_static 的 GET 检查与 `/static/` 前缀检查两处条件分支目标写反（命中反而 ret 0 → 恒 404）；③ BootRun 响应组装改**行尾 CRLF 风格**（每行自带结尾 `\r\n`、body 前单个 `\r\n` 终结）——原前缀式组装与 WithHeader 条目的尾 `\r\n` 相接成 `\r\n\r\n` 提前闭合 header，Set-Cookie/响应体被 curl 当 body 截断；④ 404 常量长度 52→54（body "Not Found" 被截 2 字节且 CL 少计致 curl 挂起等尾字节）；⑤ BootRun 对 TcpListen/TcpAccept null 加 usleep(100ms) 退避重试（端口被占不再 SIGSEGV，listener 存 alloca 槽避免 SSA 重复赋值）+ `usleep` declare。
- **已知限制**（记 TECHNICAL_DEBT）：LLVM 端单 Set-Cookie 槽（WithCookie 覆盖）、xhdrs 1024 字节上限、响应 handle/xhdrs malloc 每请求不回收（预存在行为，Go 端无此问题）。

### P3：页面框架完善（Redirect、自定义 404/500 错误页、模板上下文约定）

- **Go 后端（pkg/boot）**：`Response.Redirect(url)`（302 + `Location` header，fluent）；`Router.SetNotFoundPage/SetErrorPage`（`boot.SetNotFoundPage/SetErrorPage` 桥接，app.go 顶层封装）——404 页优先于默认 handler 文本，500 页经 handler `recover()` 兜底（panic → 500 + 自定义页/内置 "500 Internal Server Error"），锁外快照页文本避免 RLock 外读。
- **LLVM 后端（pkg/llvmgen）双端同步**：**Redirect** 降级（emitBootResponseMethodCall 新 case：store 302 到 status 槽 + xhdrs 追加 `"Location: %s\r\n"`，复用 WithHeader 的 malloc-1024-首次 + strcat 模式）；**错误页**：`@__kylix_boot_404_page/@__kylix_boot_500_page` 模块级全局（`bootErrorPagesDeclared` 声明守卫，BootRun 始终引用故无条件声明）+ `void @__kylix_boot_BootNotFoundPage/BootErrorPage(ptr)` setter define + emitBootCall/emitBootBody case + stdlib.go boot 名单；**BootRun 404 动态页**（null → 原常量响应；非 null → snprintf `Content-Length` 头（per-request arena）+ 页体两段 send）；**BootRun 500 setjmp handler**——entry 块 jmpbuf/oldJB alloca（循环内 alloca 会无界涨栈），proceedLbl 处 arm（存旧 jmpbuf → install → `setjmp`），raise 于 read/parse/handler 任意点 longjmp 回 arm 点进 catch500：恢复外层 jmpbuf + `exc_active=false` + 发 500 页（null → 内置 120 字节常量）→ closeLbl（arena reset 不漏）；正常路径 emitBootSend 后 pop handler。
- **`result.Redirect(...)` receiver 解析修复**：class.go 方法发射此前不注册 `localTypes["result"]`（stmt.go 仅注册 class 类型，TResponse 等 boot 句柄不在 g.classes）→ `result := result.Redirect('/new')` 落 "unsupported receiver" stub；新增 `isBootHandleType`（TRequest/TResponse/BootRequest/BootResponse）+ class.go/stmt.go 两处注册 result 槽 Kylix 类型。顺带修 `bootReasonPhrase` 302 → "Found"（原 302 OK）。
- **模板上下文约定（纯 Kylix 视图模型）**：TTemplateEngine 新增 `AddVariant(name, value)`（标量经 VariantToStr 字符串化入 Scalars）/`SetContext(map[String]String)`（整体替换 Scalars）/`AddListLen(name, count)`；`{{#each}}` 列表长度支持 `<name>#len` 标量键约定（TplResolveListLen 在 ListLens miss 后探 `Scalars[name + '#len']`，前缀链内探 `p + name + '#len'`）——无 map 迭代语言下 list 上下文与标量上下文统一。
- **VariantToStr 内建四端贯通**：Go 端 `fmt.Sprintf("%v", box)`；LLVM 端标量 emitVariantBox 装箱后 `__kylix_variant_as_str`（烘焙 runtime 段无条件可用）；bootstrap emitter 同款派发（LocalKylix 探测 Variant 标识符免二次装箱，复合 `and` 条件拆嵌套 if——坑清单）；bootstrap Go generator `%v` case。**连带修复 3 处 Variant 传参装箱缺口**：emitIdentLoad 先查 localTypes（方法参数 alloca 按后缀 `_str` 会误判 Variant 为 String）；普通函数调用/虚调用/inherited 三处调用点对 Variant 形参标量实参 emitVariantBox 装箱（原 i64 直传导致 llc IR verify 报错）。
- **测试与验证**：pkg/boot +5 测试（Redirect/404 页/500 页 recover/两默认兜底）+ llvmgen +3 IR 测试（错误页全局与 setter/setjmp arm/Redirect 302）；16 包全绿；**E2E curl 四场景**（302+Location、200 页、Custom 404、Custom 500 panic→500 页）全过；bootstrap sweep **52 PASS + 0 FAIL + 1 SKIP**（example59 模板教程多文件首入全绿）；**IR 不动点保持**（gen1 ≡ gen2，~234k 行逐字节）。**已知限制**：bootstrap 端 boot server/错误页不支持（boot 系列仅 BootText/BootHTML stub，与 P2 BootStatic 一致）；v0.5.x Go 自繁殖路径（gen1.go `go build`）对 v0.6.9+ 新增的 `X as TBlockStatement`（字段已具体类型）发射类型断言编译错误——Go-free 闭环不受影响，记 TECHNICAL_DEBT。
