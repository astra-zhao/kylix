...
# clean_artifacts removes .ll/.o generated next to the source by the LLVM
# pipeline, but ONLY untracked ones — committed artifacts (e.g. the historical
# example01_hello.ll checked into the repo) are preserved.
clean_artifacts() {
    local base="$1"
    for ext in .ll .o; do
        local f="${base}${ext}"
...
run_single_file_dir() {
    ...
    for f in "${files[@]}"; do
        TOTAL=$((TOTAL + 1))
        local name="${f%.klx}"
        if $KYLIX build --backend=llvm -o "$BINDIR/$name" "$f" >/dev/null 2>&1; then
            if [ -x "$BINDIR/$name" ] && "$BINDIR/$name" >/dev/null 2>&1; then
                echo "  ✓ $f"
                PASS=$((PASS + 1))
            else
                echo "  ✗ $f (run failed)"
        ...
        # build --backend=llvm writes .ll/.o next to the source — clean up
        # only the ones this run created (untracked).
...
run_module_test() {
    ...
    TOTAL=$((TOTAL + 2))
    if $KYLIX build --backend=llvm -o "$BINDIR/example33_use_module" \
            math_helper.klx example33_use_module.klx >/dev/null 2>&1; then
        if [ -x "$BINDIR/example33_use_module" ] && "$BINDIR/example33_use_module" >/dev/null 2>&1; then
