...
- [ ] **多返回函数中 `result := (a, b)` 被无条件翻译成 `return`**：Go 后端对多返回函数体内的元组赋值一律发 `return a, b`，不看位置——写在函数**开头**的初始化赋值会把其后所有语句变成死代码（regex 引擎 RECompile/REFindAt 首版全 F 的根因）。规避模式：嵌套 if 守卫 + 末尾单点/双分支 `result := (…)` 赋值。修复方向：仅当元组赋值位于函数出口（或后继无语句）时才发 `return`，否则发 `a, b = …` 多重赋值
- [ ] **多返回函数中 `exit` 翻译成空操作**（静默跳过）：`exit` 在多返回函数体内生成空语句（Go 后端不知 exit 该返回什么元组）——`if bad then exit;` 的守卫静默失效。规避模式同上（嵌套 if + 末尾单点赋值，绝不用 exit）。修复方向：exit 在多返回函数中报编译错误（比静默跳过好），或要求显式 `exit(x, y)`
- [ ] **跨单元零参函数调用丢括号**：`Sprintf("%d", ZeroArgFnFromOtherUnit)` 把函数值当参数打印（不发调用括号）——同单元/类方法不受影响。regex 引擎所有公开函数均 ≥1 参数规避。修复方向：标识符解析时查跨单元函数表，已知函数名补 `()`
- [ ] **`SetLength(arr, n)` 三端支持不完整**：bootstrap emitter 无 SetLength（regex 引擎全程用 `append` 规避）；数组结果在 procedure 内局部接收（数组作参数传递另有形态限制）。与既有「array-of-String 方法参数链式传递不可 lower」同族
...
- [x] **~~v0.5.x Go 自繁殖路径（gen1.go → go build）已腐化~~（v0.7.2 处置完毕）**：host Go 后端对 9 文件清单（含 v0.6.9 新增的 stdlib_ir.klx）生成 Go 代码编译不过（`StdIrInit` undefined + 多态 gate 不一致 7 处：`*TBlockStatement is not an interface`）。v0.6.9 起 bootstrap 闭环是 Go-free 的，此路径无存在价值——**v0.7.2 处置：(1) bootstrap `src/main.klx` 非 `--emit-llvm` 调用改为显式报错 `error: the bootstrap Go-codegen path is retired; use --emit-llvm`；(2) CI selfrepro job 改走 `--emit-llvm` IR 链（gen1 ≡ gen2 逐字节不动点，真验证）**。`src/generator.klx` 的 Go emitter 代码保留（自举编译器组成部分），只是不再从 main.klx 调用。若未来恢复：Go 路径需跳过/适配 stdlib_ir.klx + 修 `as` 发射（操作数类型已知具体时发直赋值）
- [x] **~~bootstrap Go codegen 路径（main_self 无 --emit-llvm）在 Linux 上输出损坏~~（v0.7.2 随路径退役一并处置）**：Linux 构建的 main_self 对含引号字符串常量的程序生成损坏 Go 代码（`fmt.Println("Hello")` → `@p[ :=`）。该路径自 v0.6.9 起不再参与验证，v0.7.2 已在 bootstrap `src/main.klx` 显式退役（非 `--emit-llvm` 调用直接报错），release.yml 诊断步骤改为验证报错行为。损坏根因不再需要排查
...
- [x] **~~boot/stdlib 函数"三处名单"同步陷阱~~（v0.7.2 单源化）**：Boot* API 名单收归 `internal/bootapi.BootFunctions` 单一来源，host Go（generator_stdlib.go）与 LLVM（stdlib.go）两端 import 同一份表——名单漂移成为编译错误。顺带修复 LLVM jwt 模块名单缺 `BootRegisterJwtAuth`（host 端有）的漂移残留。dispatch 侧（emitBootCall case / bootStubReturnTypes）仍手写，由 `TestBootNames_Dispatchable` 单测守护；bootapi 本身由 `TestBootNames_Sane` 守护（无重复/前缀/排序/jwt 交叉项）。v0.7.1 P4 的 host-vs-LLVM 源文本对比测试（TestBootNameLists_Converged）随单源化退役
...
- [x] **~~LLVM 端类方法多返回不支持~~（v0.7.2 已实现）**：类方法 `(A, B)` 多返回走 vtable 槽聚合返回——预扫描注册 `%__ret_<Class>_<Method>` 命名 struct + `multiRetTypes`（pkg/llvmgen/codegen.go）；`MethodInfo.MultiRetTypes`（继承/override 传递）；emitMethod define 聚合返回 + `result` 槽标 `__tuple__`（`result := (a,b)` 走 emitTupleBuild insertvalue 链）；emitVirtualCall 间接调用返回聚合；`(q, r) := obj.M()`（emitTupleDestructure）与 `var q, r := obj.M()`（emitVarDecl 新感知，与 host v0.7.0 P0 parity）两种解构形式均支持，端到端输出与 host Go 端一致。模板引擎 `RenderString`/`ErrorMsg` 绕行模式可退役（未改，教程 59 已工作）
- [ ] **类内签名 + 类外实现（Pascal 经典风格）在 LLVM 端双 define 冲突**（v0.7.2 类方法多返回开发中发现，单返回同样存在）：`type TCalc = class function F(): Integer; end;` + 类外 `function TCalc.F...` 实现——emitMethod（Body==nil stub）与 emitFunctionDecl（`TCalc_F`）各发一个同名 `@TCalc_F` define → llc `invalid redefinition`。教程全用类内直接实现所以从未暴露。规避：方法体写在类内（教程风格）。修复方向：类内签名（Body==nil）时若存在同符号外部实现则跳过 stub（或发 declare）
