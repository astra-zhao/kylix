...
实测链路：
```
宿主 kylix → src/*.klx → kylix_self (gen1, 0 错)
kylix_self → src/*.klx → self_7.go (5390 行, 0 错) → kylix_self2 (gen2, 2.9MB)
kylix_self2 → hello.klx → 可运行 Go → "Hello, World!"  ✅ round-trip
kylix_self2 → src/*.klx → self_7_gen2.go (5390 行) → kylix_self3 (gen3)  ✅ 自繁殖
kylix_self3 → hello.klx → 正确输出  ✅ 编译器能编译自己
```
...
### 7.2 v5.4 目标：自举通用 codegen 补全

v5.3 让自举编译器能正确编译**自举源码自身用到的特性子集**。自举源码不用的特性仍 stub/缺失（见 [TECHNICAL_DEBT.md](../TECHNICAL_DEBT.md) v5.4 缺口表 G1–G27）：

- 字符串插值（G2）、match（G7）、try/except/finally（G8）、lambda/await/tuple（G11）、async 函数（G15）、variant 类型（G16）、多返回值（G6）。
- stdlib 模块派发（G3，`uses web/orm/jsonutil/...`）、KylixBoot/Validation/ORM 注解自动装配（G22-G24）。
- CollectClassTypes 填充（G1/G5/G25——自举源码不用多态构造故自编译不触发，但编译其它有多态程序时会）。
- MapBuiltinFunction 补全（G4）、TFloatLiteral 真值（G10）、Body==nil 前向声明跳过（G17）、//line 指令（G19）。

**v5.4 验收**：`kylix_self2` 能正确编译用了上述特性的其它程序（不只是自举源码自身）。
...
- [ ] 改完跑 §6 三套验证（单元测试 + 自举构建 + 教程回归）+ §7.1 round-trip 链路
- [ ] 确认无游离 `main.go` 残留在仓库根（会导致根包构建失败）
- [ ] 文件行数 ≤1000（项目约束，针对 `.go`；自举 `.klx` 不受限但应控制增长）
