...
> **Status**: v0.7.0 — `--llvm-opt` runs the standalone `opt` tool for full IR-level optimization (mem2reg, inlining, loop induction, DCE), in addition to llc's codegen-level `-O<N>`. Since v0.6.5 the default `-O0` build already runs `opt --passes=mem2reg` (see below); since v0.6.2 the backend supports `--target` cross-compilation.
>
