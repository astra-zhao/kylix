...
> **Tutorial Coverage**: 54/54 tutorials compile to native binary (100%, incl. example60 server E2E); bootstrap no-Go loop achieved (IR fixed point, gen1 ≡ gen2 ≡ gen3)
...
- [x] **No-Go closed loop (v0.6.9)**: gen1 (host) emits the compiler's own IR via `--emit-llvm` (9 files) → `opt -passes=mem2reg` → `llc` → `clang` → native **gen2** compiler (zero Go dependency) → gen2 re-emits the same 9 files **byte-identical to gen1** (~220k lines — true IR fixed point), gen3 ≡ gen2. The stdlib ships to the bootstrap as baked IR data (`src/stdlib_ir.klx`, generated by `scripts/extract_stdlib_ir.py`). Tutorial sweep: **52 PASS + 2 SKIP** (`scripts/test_bootstrap_all.sh`, bootstrap-vs-host byte diff; example15 lambda / example50 jwt closed in v0.6.9 P4.12).
- [~] Custom runtime KylixRT: native binaries run on libc + hand-rolled runtime (Variant box, htab, arena for datetime). **Memory management (v0.10.0)**: `--gc=boehm` routes user-data allocations (classes/records/strings/arrays/Variant boxes/closures) through the Boehm conservative GC, so long-running programs reclaim garbage like the Go backend (opt-in; links `-lgc`, macOS `brew install bdw-gc`, Linux `libgc-dev`; not yet supported for the windows target). The default mode remains malloc-based with no free — fine for short-lived programs, but user allocations in long-running programs are only reclaimed at process exit.
